...
## 📊 Holodex vs yt-dlp: 왜 검출 결과가 다를까?

Kronii(ourokronii) 기준으로 실제 스캔 결과를 비교해 보겠습니다.

### 검출된 출연 수

| 방식 | 검출 수 | 소요 시간 |
|------|---------|----------|
| Holodex API | **76개** | 1~3초 |
| yt-dlp (description 스캔) | 11개 | 수십 분 |

### yt-dlp가 못 찾은 이유

#### 1. yt-dlp가 채널당 가져오는 영상 수가 너무 적음

yt-dlp 방식은 각 채널에 접속해서 영상 목록을 가져오는데, **타임아웃(180초)** 이 걸리거나 중간에 끊겨서 실제로 가져오는 영상 수가 매우 적습니다:

| 채널 | yt-dlp 수집 | 실제 존재하는 영상 |
|------|-------------|-------------------|
| @holoen_raorapanthera | **6개** | 수백 개 |
| @holoen_ceciliaimmergreen | **8개** | 수백 개 |
| @holoen_gigimurin | **17개** | 수백 개 |
| @fuwamococh | **21개** | 수백 개 |
| @shiorinovella | **40개** | 수백 개 |

6개~40개 영상 중에서 Kronii 멘션이 있는 영상을 찾는 건 **운에 맡기는 것**과 같습니다.

#### 2. 일부 채널은 아예 0개

yt-dlp가 아예 접속 실패해서 0개를 반환한 채널들:

| 채널 | yt-dlp 수집 | 결과 |
|------|-------------|------|
| @hololive | **0개** | 모든 공식 채널 영상 누락 |
| @hololiveenglish | **0개** | 모든 공식 채널 영상 누락 |
| @takanashikiara | **0개** | Kiara 채널의 모든 콜라보 누락 |

이 세 채널만 해도 최소 10개 이상의 Kronii 콜라보 영상이 있었지만 전부 놓쳤습니다.

#### 3. description 기반 검색의 한계

모든 콜라보 영상의 설명(description)에 `@ourokronii`가 포함되어 있지는 않습니다.
특히 **단체 콜라보(ENigmatic Recolection 등)** 는 설명에 모든 참가자를 다 적지 않는 경우가 많습니다.

### Holodex가 더 잘 찾는 이유

1. **이미 구축된 데이터베이스 사용**: Holodex는 유튜브 전체를 크롤링해서 모든 영상의 콜라보 관계를 데이터베이스화했습니다.
2. **채널 ID 기반 검색**: `UCmbs8T6MWqUHP1tIQvSgKrg`(Kronii)를 멘션한 모든 영상을 **순식간에 조회**합니다.
3. **멘션뿐만 아니라 다양한 신호 사용**: 제목, 설명, 태그, 댓글 등 여러 신호를 종합해서 콜라보를 판단합니다.
4. **타임아웃 없음**: API 한 번 호출로 모든 데이터를 가져오므로 중간에 끊길 일이 없습니다.

### 결론

| 항목 | yt-dlp | Holodex API |
|------|--------|-------------|
| 속도 | 채널당 30초~5분 (총 몇 시간) | **채널당 1초** (총 몇 초) |
| 커버리지 | 제한적 (타임아웃, 수집 한계) | **완전함** (전체 DB) |
| 정확도 | 설명 텍스트에 의존 | **멘션 + 메타데이터 종합** |
| 신규 멤버 | 새 채널 수동 추가 필요 | **자동** (채널 ID만 있으면 됨) |

> 💡 **yt-dlp 방식은 레거시로만 유지됩니다.** 새로운 스캔은 항상 `--source holodex` (기본값)를 사용하세요.
